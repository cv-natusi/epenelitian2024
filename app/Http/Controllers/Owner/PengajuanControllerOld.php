...
class PengajuanController extends Controller
{
  ...
  public function datagridterima(Request $request)
  {
    ...
    for ($i=0; $i < count($data['rows']); $i++) {
      ...
      $get_status_doc = Doc_Pendukung::where('permohonan_id',$data['rows'][$i]->id_permohonan)
      ->where('doc_status','Pending')->get();
      $terima = Doc_Pendukung::where('permohonan_id',$data['rows'][$i]->id_permohonan)
      ->where('doc_status','Terima')->get();
      $tolak = Doc_Pendukung::where('permohonan_id',$data['rows'][$i]->id_permohonan)
  ...
  public function approveFile(Request $request)
  {
    ...
    $File = Doc_Pendukung::find($id_file);
    ...
    if($File){
      ...
      $tolak = Doc_Pendukung::where('permohonan_id',$users->id_permohonan)->where('doc_status',  'Tolak')->get();
      ...
      if(count($tolak) > 0){
        ...
        $jumlah_syarat = Doc_Pendukung::where('permohonan_id',$users->id_permohonan)->get();
        $permohonan = Permohonan::find($id_permohonan);

          if (Auth::getUser()->level == 1) {
            $terima = Doc_Pendukung::where('permohonan_id',$users->id_permohonan)->whereRaw("acc_admin='Terima'")->get();
            ...
            $terima = Doc_Pendukung::where('permohonan_id',$users->id_permohonan)->whereRaw("acc_kasi='Terima'")->get();
            if(count($terima)==count($jumlah_syarat)){
              $status_permohonan = 'Terima'; 
              $data = [
                'nama_pemohon' => $users,
                'file_nama' => Doc_Pendukung::join('jenis_file_pendukung','doc_pendukung.jenis_file', '=', 'jenis_file_pendukung.id_jenis_file')->where('permohonan_id',$users->id_permohonan)->orderBy('jenis_file','Asc')->get(),
              ];
              // return $data;
            ...
            $terima = Doc_Pendukung::where('permohonan_id',$users->id_permohonan)->whereRaw("acc_kabid='Terima'")->get();
            if(count($terima)==count($jumlah_syarat)){
              $status_permohonan = 'Menunggu Kadin';
            }
          }else {
            $terima = Doc_Pendukung::where('permohonan_id',$users->id_permohonan)->whereRaw("acc_kadin='Terima'")->get();
            if(count($terima)==count($jumlah_syarat)){
              $status_permohonan = 'Terima';
            }
          }
        ...
        $permohonan->save();                
        return ['status' => 'success','id_file'=>$id_file,'file_doc'=>$File];        
      }         
    }
