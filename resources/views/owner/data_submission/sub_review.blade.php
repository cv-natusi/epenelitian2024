...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		...
		columns                 : [			
			{field: 'id', title: 'SUB ID', editable: false, sortable: true, width: 50, align: 'left', search: true},		
			{field: 'users_id', title: 'USER ID', editable: false, sortable: true, width: 50, align: 'left', search: true},		
			{field: 'title', title: 'Title', editable: false, sortable: true, width: 150, align: 'left', search: true},
			{field: 'first_name', title: 'First Name', editable: false, sortable: true, width: 150, align: 'left', search: true},
			{field: 'id_orcid', title: 'ORCID ID', editable: false, sortable: true, width: 150, align: 'left', search: true},			
	...
	function menu(rowData, rowIndex) {
        var menu =
	        ...
	        '<li onclick="detailJournal(' + rowIndex + ')"><a href="javascript:void(0);"><i class="fa fa-eye"></i> View File</a></li>' +
	        '<li onclick="detReview(' + rowIndex + ')"><a href="javascript:void(0);"><i class="fa fa-pencil"></i> Details Review</a></li>' +
 ...
 //        var rowData = datagrid.getRowData(rowIndex);
	...
	// 	});	
	// }
	function detReview(rowIndex){
        var rowData = datagrid.getRowData(rowIndex);
		$('.loading').show();
		$('.main-layer').hide();
        // swal("Good",rowData.judul,"success");
        $.post("{!! route('detReview') !!}",{id:rowData.id}).done(function(data){
			$('.loading').hide();
			if(data.status == 'success'){
				$('.other-page').html(data.content).fadeIn();
			} else if(data.status=='fail'){
    ...
    function form(id){
      $.post("{{route('detailAuth')}}",{id:id},function(data){
        if(data.status=='success'){
