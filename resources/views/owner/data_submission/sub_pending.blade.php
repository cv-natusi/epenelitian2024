...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		...
		columns                 : [			
			...
			{field: 'abstract', title: 'Abstract', editable: false, sortable: true, width: 150, align: 'left', search: true},			
	...
	function menu(rowData, rowIndex) {
        var menu =
	        ...
	        '<li onclick="reviewJournal(' + rowIndex + ')"><a href="javascript:void(0);"><i class="fa fa-pencil"></i> Review Journal</a></li>' +
 ...
 //        var rowData = datagrid.getRowData(rowIndex);
	...
	// 	});	
	// }
	function reviewJournal(rowIndex){
        var rowData = datagrid.getRowData(rowIndex);
		$('.loading').show();
		$('.main-layer').hide();
        // swal("Good",rowData.judul,"success");
        $.post("{!! route('reviewJournal') !!}",{id:rowData.id}).done(function(data){
			$('.loading').hide();
			if(data.status == 'success'){
				$('.other-page').html(data.content).fadeIn();
			} else if(data.status=='fail'){
