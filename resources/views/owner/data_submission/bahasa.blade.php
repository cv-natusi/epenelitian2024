...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		url                     : "{!! route('datagridbahasa') !!}",
		primaryField            : 'bhs.id_bahasa', 
		rowNumber               : true, 
		rowCheck                : false, 
		...
		columns                 : [			
			{field: 'id_bahasa', title: 'ID Bahasa', editable: false, sortable: true, width: 50, align: 'left', search: true},
			{field: 'inggris', title: 'B Inggris', editable: false, sortable: true, width: 50, align: 'left', search: true},
			{field: 'indonesia', title: 'B Indonesia', editable: false, sortable: true, width: 150, align: 'left', search: true},
	...
	function menu(rowData, rowIndex) {
        var menu =
	        ...
	        '<li onclick="update_bahasa(' + rowIndex + ')"><a href="javascript:void(0);"><i class="fa fa-pencil"></i> Update</a></li>' +
	        '<li onclick="dohapus(' + rowIndex + ')"><a href="javascript:void(0);"><i class="fa fa-trash-o"></i> Delete</a></li>' +
	        '</ul>' +
	        '</div>';
        return menu;
    }

	function update_bahasa(rowIndex){
        var rowData = datagrid.getRowData(rowIndex);
		$('.loading').show();
		$('.main-layer').hide();
        // swal("Good",rowData.judul,"success");
        $.post("{!! route('update_bahasa') !!}",{id:rowData.id_bahasa}).done(function(data){
			$('.loading').hide();
    ...
    function dohapus(rowIndex){
        var rowData = datagrid.getRowData(rowIndex);
		swal({
			title:"Hapus text ini",
		...
		function(){
			$.post("{!! route('dohapus') !!}",{id:rowData.id_bahasa}).done(function(data){
				if(data.status == 'success'){
					datagrid.reload();
					swal("Success!", "Author telah dihapus !", "success");
				}else if(data.status=='fail'){
					datagrid.reload();
