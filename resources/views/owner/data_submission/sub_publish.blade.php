...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		...
		columns                 : [			
			...
			{field: 'comments', title: 'Comments', editable: false, sortable: true, width: 150, align: 'left', search: true},			
	...
	function menu(rowData, rowIndex) {
        var menu =
	        ...
	        '<li onclick="detail_publish(' + rowIndex + ')"><a href="javascript:void(0);"><i class="fa fa-pencil"></i> Details Journal</a></li>' +
 ...
 //        var rowData = datagrid.getRowData(rowIndex);
	...
	// }
	 ...
	 function detail_publish(rowIndex){
    var rowData = datagrid.getRowData(rowIndex);
      $('.loading').show();
      $('.main-layer').hide();
          // swal("Good",rowData.judul,"success");
          $.post("{!! route('detail_publish') !!}",{id:rowData.id}).done(function(data){
        $('.loading').hide();
        if(data.status == 'success'){
          $('.other-page').html(data.content).fadeIn();
        } else if(data.status=='fail'){
